...
int ParsingFrameStack::check(ParsingFrame &frm, int &pp_level, Chunk *pc)
{
   ...
   if (pc->TestFlags(PCF_IN_PREPROC))
   {
      ...
      if (pc->GetParentType() == E_Token::CT_PP_IF)
      {
         ...
         pp_level++;
         push(frm);
         frm.SetIfdefType(E_Token::CT_PP_IF);
         txt = "if-push";
      }
      else if (pc->GetParentType() == E_Token::CT_PP_ELSE)
      {
         ...
         if (frm.GetIfdefType() == E_Token::CT_PP_IF)
         {
            // we have [...] [base]-[if], so push an [else]
            push(frm);
            frm.SetIfdefType(E_Token::CT_PP_ELSE);
            if_block = true;
         }
      ...
      else if (pc->GetParentType() == E_Token::CT_PP_ENDIF)
      {
         ...
         if (frm.GetIfdefType() == E_Token::CT_PP_ELSE)
         {
         ...
         else if (frm.GetIfdefType() == E_Token::CT_PP_IF)
         {
            /*
             * We have: [...] [base] [if]
             * We want: [...] [base]
